...
export default class Images {
    ...
    constructor(images, nameGroup, withCover) {

        this.container = images;
        this.cover = withCover;
        this.group = nameGroup;
        let reg = new RegExp('id="form_'+this.group+'.*?"', "g");
        this.template = images.data('prototype').replace(reg, '');
    ...
    render() {
        const images = Object.keys(this.images).map((index) => {
            ...
            image.find('input[name="form['+this.group+'][' + index + '][id]"]').val(this.images[index].id);
            image.find('input[name="form['+this.group+'][' + index + '][src]"]').attr('value', this.images[index].src);
            if (this.cover) {
                image.find('input[name="form['+this.group+'][' + index + '][cover]"]').attr('checked', Boolean(this.images[index].cover));
            }
            return image.html();
        });
