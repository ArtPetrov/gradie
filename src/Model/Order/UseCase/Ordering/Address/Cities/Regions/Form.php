<?php

declare(strict_types=1);

namespace App\Model\Order\UseCase\Ordering\Address\Cities\Regions;

use Symfony\Component\Form\AbstractType;
use Symfony\Component\Form\Extension\Core\Type;
use Symfony\Component\Form\FormBuilderInterface;
use Symfony\Component\OptionsResolver\OptionsResolver;

class Form extends AbstractType
{
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        $builder
            ->add('city', Type\TextType::class, ['required' => false])
            ->add('address', Type\TextType::class, ['required' => false])
            ->add('transportCompany', Type\TextType::class, ['required' => false])
            ->add('tightPacking', Type\CheckboxType::class, ['required' => false])
            ->add('insurance', Type\CheckboxType::class, ['required' => false])
            ->add('deliveryTo', Type\ChoiceType::class, [
                'choices' => [
                    'До терминала транспортной компании' => 'company',
                    'До Вашего адреса (курьером транспортной компании)' => 'home'
                ],
                'expanded' => true
            ])
            ->add('freeShippingLimit', Type\HiddenType::class)
            ->add('baseCostShipping', Type\HiddenType::class);

    }

    public function configureOptions(OptionsResolver $resolver): void
    {
        $resolver->setDefaults([
            'data_class' => Command::class,
        ]);
    }
}
